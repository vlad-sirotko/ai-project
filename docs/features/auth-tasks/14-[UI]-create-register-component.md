...
## Description
Create the `RegisterComponent` for the `/auth/register` route. It renders an email, password, and confirm-password form with client-side validation and delegates submission to `RegisterFacade`. On success the facade redirects the user to `/app/upload`.
...
- [x] Calls `RegisterFacade.register()` on submit; button disabled while request is in-flight
...
- [x] `RegisterFacade` provided at component level via `providers: [RegisterFacade]`

## Technical Notes
- Layer: UI
- Key files: `frontend/src/app/features/auth/register/register.component.ts`, `.html`, `.scss`, `register.facade.ts`
- Depends on: `12-[UI]-implement-auth-store.md`, `10-[UI]-set-up-auth-routing.md`

## Implementation Notes
- `RegisterFacade` and `FormBuilder` injected via `inject()` in the component — `AuthStore` and `Router` are encapsulated in the facade
- `RegisterFacade` is `@Injectable()` (no `providedIn`) and registered in `providers` on the component so it is scoped to the component instance and easily mockable in tests
- `RegisterFacade.register()` calls `AuthStore.register()` then `Router.navigate(['/app/upload'])`
