...
## Acceptance Criteria
- [ ] `AuthService` is an `@Injectable({ providedIn: 'root' })` service
- [ ] `register(email, password)` calls `POST /api/auth/register` and returns `AuthResponse`
- [ ] `login(email, password)` calls `POST /api/auth/login` and returns `AuthResponse`
- [ ] `getMe()` calls `GET /api/auth/me` and returns `UserModel`
- [ ] `updatePreferences(language)` calls `PUT /api/auth/me/preferences`
- [ ] `UserModel` and `AuthResponse` interfaces defined in `frontend/src/app/shared/models/`
- [ ] No side effects (state management) inside the service — that belongs in `AuthStore`
