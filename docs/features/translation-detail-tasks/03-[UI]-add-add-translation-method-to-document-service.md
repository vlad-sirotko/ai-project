...
## Acceptance Criteria
- [ ] `DocumentService.addTranslation(documentId: string, targetLang: string): Observable<TranslationJobModel>` method added
- [ ] Posts `{ targetLanguage }` JSON body to `/api/documents/{documentId}/translate`
- [ ] Response is typed as `TranslationJobModel` (verify `TranslationJobModel` has all fields returned by the API: `id`, `targetLanguage`, `status`, `translatedText`, `errorMessage`, `createdAt`, `completedAt`)
- [ ] No changes to existing `DocumentService` methods
